...
namespace ItinerariesAdminWebApp.Models.Gateway
{
    public class SendGridMailer : IMailSender
    {
        ...
        public async Task<bool> Send(string to, string subject, string body)
        {
            ...
            var msg = MailHelper.CreateSingleEmail(_fromAddress, toEmailAddress, subject, body, "");
            var response = await client.SendEmailAsync(msg);
            return response.StatusCode == System.Net.HttpStatusCode.Accepted;
        }

        public async Task<bool> Send(List<string> toList, string subject, string body)
        {
            var client = new SendGridClient(_apiKey);
            var tos = toList.Select(email => new EmailAddress(email)).ToList();
            var msg = MailHelper.CreateSingleEmailToMultipleRecipients(_fromAddress, tos, subject, body, "");
            var response = await client.SendEmailAsync(msg);
            return response.StatusCode == System.Net.HttpStatusCode.Accepted;
        }
    }
